...
const FINANCEIRO_FUNCOES = ["financeiro", "gerencia_financeira"];
...
const BTN_PRI   = "w-full bg-zinc-900 hover:bg-zinc-800 disabled:bg-zinc-300 text-white py-2.5 rounded-lg text-sm font-medium transition-colors";
...
function diffDias(iso: string): number {
  ...
  return Math.floor((new Date(iso + "T12:00:00Z").getTime() - hoje.getTime()) / 86400000);
}
...
function ConfirmarModal({ etapa, userEmail, onClose, onDone }: {
  etapa: Etapa; userEmail: string; onClose: () => void; onDone: () => void;
}) {
  ...
  const handle = async () => {
    ...
    try {
      ...
      if (file) {
        ...
        const path = `${etapa.id}/${Date.now()}.${file.name.split(".").pop()}`;
        const { error } = await sb.storage.from(SUPABASE_BUCKET).upload(path, file, { upsert: true });
        if (error) throw new Error(error.message);
        const { data: { publicUrl } } = sb.storage.from(SUPABASE_BUCKET).getPublicUrl(path);
        url  = publicUrl;
        nome = file.name;
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-zinc-500 mb-1.5">Comprovante (PDF ou Excel) <span className="text-red-500">*</span></label>
            <div className="border-2 border-dashed border-zinc-200 rounded-xl p-5 text-center cursor-pointer hover:border-orange-300 transition-colors" onClick={() => fileRef.current?.click()}>
              {file ? <p className="text-sm text-zinc-700 font-medium">{file.name}</p> : (
                <>
                  <svg className="mx-auto mb-2 text-zinc-300" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                  <p className="text-sm text-zinc-400">Clique para selecionar</p>
                  <p className="text-xs text-zinc-300 mt-0.5">PDF, XLS, XLSX</p>
                </>
              )}
            </div>
            <input ref={fileRef} type="file" accept=".pdf,.xls,.xlsx" className="hidden" onChange={e => setFile(e.target.files?.[0] ?? null)} />
          </div>
          <button onClick={handle} disabled={loading || !file} className={BTN_PRI}>{loading ? "Confirmando..." : !file ? "Selecione o comprovante" : "Marcar como Concluído ✓"}</button>
        </div>
      </div>
    </div>
  );
...
function JustificarModal({ etapa, userEmail, onClose, onDone }: {
  etapa: Etapa; userEmail: string; onClose: () => void; onDone: () => void;
}) {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea value={texto} onChange={e => setTexto(e.target.value)} rows={4} className={`${INPUT} resize-none`} placeholder="Descreva o motivo..." />
          </div>
          <button onClick={handle} disabled={loading || !texto.trim()} className={BTN_PRI}>{loading ? "Enviando..." : "Enviar Justificativa"}</button>
        </div>
      </div>
    </div>
  );
...
function EtapaCard({ etapa, userEmail, userFuncao, onRefresh }: { etapa: Etapa; userEmail: string; userFuncao: string; onRefresh: () => void }) {
  ...
  const canConfirm = userFuncao === "desenvolvedor" || FINANCEIRO_FUNCOES.includes(userFuncao);
    ...
    <>
      <div className={`bg-white border rounded-xl p-4 shadow-sm ${
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium ${s.badge} mb-1`}>
        ...
        {conf && (
          <div className="bg-green-50 rounded-lg px-3 py-2 text-xs text-green-700 flex items-center justify-between gap-2">
            <span>Confirmado por <strong>{conf.confirmado_por}</strong></span>
            {conf.arquivo_url && <a href={conf.arquivo_url} target="_blank" rel="noreferrer" className="underline font-medium text-green-800">{conf.arquivo_nome ?? "Ver arquivo"}</a>}
          </div>
        )}
        {just && !conf && (
          <div className="bg-red-50 rounded-lg px-3 py-2 text-xs text-red-700">
            <span className="font-medium">Justificativa:</span> {just.justificativa}
          </div>
        )}

        {etapa.status !== "concluido" && canConfirm && (
          <div className="flex gap-2 mt-3">
            <button onClick={() => setModal("confirmar")} className="flex-1 bg-zinc-900 hover:bg-zinc-800 text-white text-xs font-medium py-2 rounded-lg transition-colors">Marcar OK</button>
            {etapa.status === "atrasado" && !just && (
              <button onClick={() => setModal("justificar")} className="flex-1 bg-red-50 hover:bg-red-100 text-red-700 text-xs font-medium py-2 rounded-lg border border-red-200 transition-colors">Justificar Atraso</button>
            )}
          </div>
        )}
      </div>

      {modal === "confirmar" && <ConfirmarModal etapa={etapa} userEmail={userEmail} onClose={() => setModal(null)} onDone={() => { setModal(null); onRefresh(); }} />}
      {modal === "justificar" && <JustificarModal etapa={etapa} userEmail={userEmail} onClose={() => setModal(null)} onDone={() => { setModal(null); onRefresh(); }} />}
    </>
  );
}

export default function FolhaFinanceiroPage() {
  ...
  const carregar = useCallback(() => {
    setLoading(true);
    fetch(`${API}/etapas?dept=financeiro`)
      .then(r => r.json())
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-zinc-400 mt-1 text-sm">Tarefas do Financeiro</p>
        </div>
        <div className="flex items-center gap-2">
          {atrasadas > 0 && (
            <span className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 border border-red-200 text-red-700 text-xs font-medium rounded-full">
              <span className="w-1.5 h-1.5 bg-red-500 rounded-full" />{atrasadas} atrasada{atrasadas > 1 ? "s" : ""}
            </span>
          )}
          {pendentes > 0 && (
      ...
      <div className="flex items-center gap-2 mb-6 flex-wrap">
        {(["todos", "pendente", "atrasado", "concluido"] as const).map(v => (
          <button key={v} onClick={() => setFiltro(v)} className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${filtro === v ? "bg-zinc-900 text-white border-zinc-900" : "bg-white text-zinc-500 border-zinc-200 hover:border-zinc-300"}`}>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-zinc-400 text-sm">
          {filtro === "todos" ? "Nenhuma tarefa gerada ainda." : "Nenhuma tarefa com esse status."}
        </div>
      ) : (
        <div className="space-y-3">
          {filtradas.map(e => <EtapaCard key={e.id} etapa={e} userEmail={email} userFuncao={funcao} onRefresh={carregar} />)}
        </div>
      )}
    </div>
  );
